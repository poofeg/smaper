...
public class VirtualTree extends VirtualList
{
	...
	public void setCLCI()
	{
		...
		ContactListItem item = (ContactListItem)currItem.getData();
		if (item instanceof ContactListContactItem)
		{
			ContactListContactItem lastChatItem = (ContactListContactItem)item; 
	...
	private static int drawNodeRect(
		Graphics g,
		TreeNode item,
		int x,
		int y1,
	...
	protected void drawItemData(Graphics g, int index, int x1, int y1, int x2, int y2, int fontHeight)
	{
		...
		super.drawItemData(g, index, x, y1, x2, y2, fontHeight);
