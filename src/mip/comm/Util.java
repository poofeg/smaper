...
public class Util
{
	...
	public static final int SET_STATUS_DEPRESS = 0x4000;
  ...
  static public byte[] explodeToBytes(String text, char serparator, int radix)
	...
	public static int translateStatusReceived(int status)
	{
		 if ((status & 0x0100) != 0 && (status & 0xFFFF) != 0x0100 && status != 0xFFFFFFFF) status &= 0xFFFFFEFF;
          ...
          if ((status & ContactList.STATUS_DEPRESS) == ContactList.STATUS_DEPRESS) return (ContactList.STATUS_DEPRESS);
	...
	public static int translateStatusSend(int status)
	{
		...
		if (status == ContactList.STATUS_DEPRESS) return (Util.SET_STATUS_DEPRESS);
    ...
    {
        a_int_static_fld = a_int_static_fld + 1 & 0x7fff;
...
*/
	...
	/* Show date string */     	
	public static String getDateString(boolean onlyTime, long date, boolean showSecond)
	{
		...
		sb.append(Util.makeTwo(loclaDate[TIME_HOUR]))
		  ...
		  {
		  sb.append(':');
		  sb.append(Util.makeTwo(loclaDate[TIME_SECOND]));
		  }
	...
	public static String getDateString(boolean onlyTime, boolean showSecond)
	{
		return getDateString(onlyTime, createCurrentDate(false), showSecond);
	}
