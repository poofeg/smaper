...
public class SearchAction extends Action
{
    ...
    public void onEvent(int eventTuype)
    {
    	...
    	{
    	case ON_COMPLETE:
    		cont.getSearchForm().activate(SearchForm.ACTIV_SHOW_RESULTS);
    		break;
    		
    	case ON_ERROR:
            if (this.state == STATE_FIRSTRESULT_RECEIVED)
            {
            	mipException.handleException(new mipException(159, 0, true));
            }
            else
            {
            	cont.getSearchForm().activate(SearchForm.ACTIV_SHOW_NORESULTS);
            }
    		break;
    		
    	case ON_CANCEL:
    		cont.getSearchForm().activate(SearchForm.ACTIV_JUST_SHOW);
